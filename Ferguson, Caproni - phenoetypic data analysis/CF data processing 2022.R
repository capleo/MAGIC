...
setwd("~/My Drive/Maize 2022 analyses/Chlorophyll fluorescence/Raw data/")
...
output$time_post_light_on <- rep(time_post_light_on, times=nrow(output)/length(time_post_light_on))
output$time_post_light_off <- rep(time_post_light_off, times=nrow(output)/length(time_post_light_off))
output$cumulative_time <- rep(cumulative_time, times=nrow(output)/length(cumulative_time))
...
ggplot(data=QY_max, aes(x=QY_max)) + geom_density() + 
  geom_vline(xintercept=0.68, colour = "red") #density plot
...
filtered_output <- filter(output, QY_max >= 0.68)
...
ggplot(data=filtered_output, aes(x=cumulative_time, y=NPQ_values, colour=Repeat)) +
  geom_point()+
  theme_classic()+
  facet_wrap_paginate( ~ Plot, ncol= 5, nrow = 5, page = 23)

#export the filtered output to load into other script to do some fitting 

write_xlsx(filtered_output, path = "~/My Drive/Maize 2022 analyses/Chlorophyll fluorescence/NPQdata.xlsx")
